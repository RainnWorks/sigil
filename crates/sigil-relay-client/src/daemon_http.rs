...
//!
//! The footgun this module has to avoid is a GET that returns EMPTY *fast*. The
//! whole no-client-sleep design assumes the relay actually holds each GET for
//! its ~25s; if instead it resolves empty immediately (the server evicting a
//! slot when a second GET races it, a misconfigured/short hold, an edge that
//! short-circuits), re-issuing with zero delay turns the loop into a hammer
//! pinned against the relay's rate limit. So `recv` TIMES each GET: an empty
//! return that came back in materially less than the expected hold is treated
//! as "the hold did not happen" and backs off (exponential, jittered, capped)
//! before the next GET, exactly like a transient error or a 429 does. An empty
//! return that took ~the full hold is the legitimate case and re-issues
//! promptly. Any actual payload, or a real full-length hold, resets the backoff.
...
impl DaemonRelay {
    ...
    /// GET `to-daemon` once, appending any envelopes to the buffer. Returns the
    ...
    fn poll_to_daemon(&self) -> Result<DrainOutcome, TransportError> {
        let outcome = self.http.poll_slot(Slot::ToDaemon)?;
        if let DrainOutcome::Envelopes(drained) = &outcome {
            ...
            for s in drained {
                match wire::wire_to_envelope(s) {
                    ...
                    Err(e) => {
                        eprintln!("sigil relay: dropped an undecodable to-daemon envelope: {e}")
                    }
                }
            }
        }
                ...
                Ok(outcome) => {
                    ...
                    match outcome {
                        ...
                        DrainOutcome::RateLimited { retry_after } => {
                            ...
                            // relay-chosen floor.
                            eprintln!("sigil relay: to-daemon poll was rate-limited (429)");
                            backoff = self.back_off_rate_limited(backoff, retry_after, deadline);
                        }
                    }
                }
                Err(e) => {
                    // A transient error (network blip, a 5xx) must not deny a
                    // legitimate approval outright: back off and retry up to the
                    // deadline. `pop_buffered`'s own error (a poisoned mutex) is
                    // the one thing that never resolves by retrying, hence the
                    // early `?` on it above.
...
mod tests {
    ...
    /// One scripted response for a fake relay's next `to-daemon` GET.
    #[derive(Clone)]
    enum Step {
    ...
    fn a_fast_empty_return_backs_off_with_an_increasing_delay_not_an_instant_reissue() {
        ...
        let got = relay
            .recv([7u8; 32], Direction::ToDaemon, Duration::from_secs(10))
