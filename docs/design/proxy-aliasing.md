...
`op item get`, `git` reaching the SSH agent) hit Sigil **transparently**. The
...
"fails" with a non-zero exit that the caller cannot attribute to Sigil. The
caller never learns Sigil exists. That anonymity is the property Tom wants.

The mechanism is a Sigil-managed **proxy directory** placed first on `PATH`,
holding one thin alias per intercepted command. The alias re-enters the multicall
binary as `sigil <cmd> "$@"`, which gates on the phone and then execs the *real*
`<cmd>`. Everything is managed by the CLI: `sigil proxy add|remove|list|status|
doctor|env`.
...
- **Multicall dispatch** (`main.rs`): `argv[0]` stem other than `sigil` is a
  transparent alias that re-enters as `sigil <stem> <args>` via
  `shim::dispatch`. `sigil <cmd>` (non-reserved verb) and `sigil run -- <cmd>`
  funnel to the same `dispatch`.
- **The alias is a symlink**, not a shell script: `setup::install_shim_for(cmd)`
  symlinks `~/.sigil/bin/<cmd>` at the running binary. We keep symlinks on
  Unix; they are strictly better than a `#!/bin/sh exec sigil <cmd> "$@"`
...
- **proxy directory**: `~/.sigil/bin`, first on `PATH`. Sigil-managed.
...
```
  caller (agent, launcher, git)
        ...
        v
  ~/.sigil/bin/op  --- symlink --->  sigil multicall binary
        |  argv[0] stem = "op" != "sigil"  => alias mode
        v
  sigil op read op://...            (shim::dispatch)
        |
...
transparent-exec fallback, matching "behave exactly like the real tool when Sigil
is off."

## Hard problem 1: recursion

`~/.sigil/bin/op` -> `sigil op` -> must find the *real* `op`, never the alias.
...
1. Skip any candidate whose canonical path equals a Sigil binary an alias points
   at: the running binary (`current_exe()`) **or** the sibling `sigil` runtime
   ...
   **symlink** to the runtime, so `canonicalize` resolves it and excludes it in
   ...
   the `sigil-config` binary, where `current_exe()` is the manager, not the
...
2. Skip any candidate inside the proxy directory (`~/.sigil/bin`) outright. This
   ...
   non-sigil executable planted in the proxy dir would be spawned by the
...
Sigil carries an inherited counter `SIGIL_PROXY_DEPTH`. The enforcement point is
the **alias's dispatch entry**: on entry Sigil reads the counter and, past a
generous bound (proposed 40), aborts with exit 70 and a diagnostic; otherwise it
sets `SIGIL_PROXY_DEPTH = n+1` on whatever it execs next.
...
- **Daemon up:** the daemon spawns the real tool. For the up-path to be bounded
  the daemon must set `SIGIL_PROXY_DEPTH = n+1` on the child it spawns (a small
...
because Sigil cannot distinguish "the real tool" from "another alias" without
...
**Wrapper real binaries (brew/asdf/mise).** If the first non-Sigil resolution is
itself a wrapper (a mise/asdf shim), Sigil execs it and lets it do its own
indirection to the real tool. mise/asdf shims resolve the tool directly, never
back through `~/.sigil/bin`, so no loop. Sigil's contract is "exec the first
non-Sigil resolution"; version-manager indirection past that point is theirs.
...
by construction, the binary the caller would have executed without Sigil. No
heuristic ranking, no guessing.

- **Resolve-and-record at add time.** `sigil proxy add <cmd>` runs `find_real`
...
- **Override.** `sigil proxy add <cmd> --real <path>` pins an explicit real
...
Sigil only prepends a directory and changes resolution *order*. The real binary
...
- `proxy doctor` flags **an alias with no gating rule**. If `proxy add op` runs
  ...
  `sigil config ...`, or `sigil proxy remove <cmd>`."
...
|-------|--------------|------|
| zsh   | `~/.zshrc`   | `export PATH="$HOME/.sigil/bin:$PATH"` |
| bash  | `~/.bashrc` and `~/.bash_profile` (or `~/.profile`) | same |
| fish  | `~/.config/fish/config.fish` | `fish_add_path --prepend $HOME/.sigil/bin` |
...
2. **The printable env line: `sigil proxy env`.** Prints the prepend line for the
   ...
   - systemd --user unit: `Environment=PATH=%h/.sigil/bin:...` or a drop-in.
...
- **macOS / Linux (first-class).** Symlink aliases in `~/.sigil/bin`; rc-block +
...
- **Windows (designed, flagged/stubbed).** A symlink-to-multicall is unreliable;
  use a per-command `<cmd>.cmd` shim (`@echo off` + `sigil %~n0 %*`) or a hard
...
- **On deny (already implemented, shim side quiet):** the daemon's `fail_closed`
  returns **exit code 1** and the shim exits silently with it (no Sigil-branded
  ...
  is not Sigil-branded, but it is a synthetic line a real tool would not emit, so
  ...
  the human already sees the denial on the phone and in `sigil history`) is a
...
- **Residuals (anonymity is best-effort, not a security boundary):** a caller
  that inspects its own resolved `PATH` sees `~/.sigil/bin`; a protocol-fault
  error path currently prints a string containing "sigil"; blocking latency
  ...
  Sigil word from the fault path.
- **The proxy is not a containment boundary (crucial).** A caller that controls
  ...
  absolute path, prepend its real directory ahead of `~/.sigil/bin`, or unset
  `PATH` interception. The proxy's job is only to make an **unmodified,
  non-hostile** caller (an agent, a launcher, `git`) transit Sigil; it is **not**
  ...
  transit Sigil, plus the daemon-verified caller identity on those requests.
...
The management verbs mount on the **`sigil-config`** binary, not the lean
gating `sigil` binary. Rationale (Tom, 2026-07-06): a reserved `proxy` verb on
the gating binary would make a real program named `proxy` ungateable, since
`sigil <cmd>` reserves its verbs. Keeping the gating binary's reserved surface
minimal means `sigil <anything>` can gate anything; the management surface lives
on `sigil-config`, which is never on the `sigil <cmd>` hot path.

```
sigil-config proxy add <cmd> [--real <path>] [--force]
...
sigil-config proxy env [--shell zsh|bash|fish|nu]
...
`sigil shim install|add` remain as thin, hidden back-compat aliases mapping onto
...
separate thin `[[bin]]` targets (lean `sigil` gating hot-path + `sigil-config`
management). The proxy splits by **role**, not as one unit:

- **Proxy RUNTIME stays in the lean `sigil` binary**: the installed alias (a
  symlink named `<cmd>` pointing at the `sigil` runtime binary, `argv[0]`
  dispatch), `find_real` resolution, and the `SIGIL_PROXY_DEPTH` fuse. This is
  the hot path a proxied `op` hits; it must be reserved-verb-minimal so
  `sigil <anycmd>` can gate anything.
- **Proxy MANAGEMENT moves to `sigil-config`**: the `proxy add|remove|list|
  status|doctor|env` verbs (reserved-verb rationale above). config-cli mounts my
  proxy command module under `sigil-config`.
...
**Critical: the alias symlink target.** When `sigil-config proxy add op` installs
the symlink, its target must be the **`sigil` runtime binary**, not the running
`sigil-config` binary. Today (single multicall binary) `current_exe()` is
correct; post-split the installer must resolve the sibling `sigil` binary
(e.g. `current_exe().parent().join("sigil")`). Symmetrically, the drift detection
...
1. **Final core-lib / bin names**; mount my proxy command module under
   `sigil-config`; confirm the sibling `sigil` runtime binary path for the alias
   target.
...
3. **Proxy dir + PATH ownership.** `~/.sigil/bin` stays the single proxy dir and
...
4. **Two daemon-side gaps config-cli owned (both LANDED)**:
   ...
   - [landed `65d75d2`] `proxy_depth` on `Frame::Run`: the shim sends its
     ...
     MAX_DEPTH` and otherwise spawns the child with `SIGIL_PROXY_DEPTH =
...
- **find_real correctness** is the whole recursion + no-clobber story. Both
  ...
  itself a symlink into the proxy dir, a `<cmd>` copy (not symlink) of the Sigil
...
- **Order drift is a gate bypass for the honest path, not a control against a
  ...
  *unmodified* caller's command ungated; `doctor` flags it and the startup
  ...
  identity for requests that actually transit Sigil. This is the same fact as
...
4. Recursion env-guard (`SIGIL_PROXY_DEPTH`) in the shim dispatch. [proxy-owned]
5. [done] Mounted `proxy add|remove|list|status|doctor|env` under `sigil-config`
   (`cli::run_config`), flipped `alias_target` to the sibling `sigil` runtime
