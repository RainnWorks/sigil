...
| **RAI-32, 2026-08-23: on Linux, ancestor code identity is a hash of the running image, taken through the fd `/proc/<pid>/exe` opens, never by reading a path.** Before this the Linux `ProcessTable` returned `None` from `parent()` and `resolve()` unconditionally, so `walk_ancestry` produced an empty chain on every Linux call and `Caller::may_lease` never granted a window: every gated command cost a fresh approval, permanently. | `lease.rs::{measure_running_image,SysProcessTable::resolve}` (`#[cfg(target_os = "linux")]`) -- opens `/proc/<pid>/exe`, hashes the bytes off that fd, and reads the path via `/proc/self/fd/<n>` off the SAME fd rather than a second `/proc/<pid>/exe` lookup, so a pid recycled between the two calls cannot pair one process's path with a different process's digest | `lease.rs::{sys_process_table_parent_reads_the_real_ppid_from_proc,a_running_binary_measures_as_a_hash_of_its_own_bytes,renaming_over_the_exe_path_does_not_change_what_a_running_process_measures_as,the_kernel_refuses_to_overwrite_a_running_executable_in_place,a_pid_that_names_no_process_yields_no_identity_at_all,still_running_tracks_the_kernels_start_time_not_just_the_pid,proc_stat_fields_survive_a_comm_containing_parens_and_spaces}`. **Scope of the claim (implementer's statement, not a verdict, recorded in full in `docs/design/linux-code-identity.md`):** `/proc/<pid>/exe` is a kernel-held reference to the exec'd inode, not a path lookup, so this measures the RUNNING image exactly as the macOS cdhash path does, by a different mechanism -- verified here (not merely reasoned about) that a rename-over the path leaves the hash unchanged and an in-place rewrite is refused outright (`ETXTBSY`). Unlike macOS, there is no state on Linux where the path resolves but the content does not, so there is no platform-refusal branch for this attack and a self-deleting tool stays fully measured as `Content` rather than dropping to `Unmeasured`. **What this row does NOT close:** which uid the daemon runs as. Opening `/proc/<pid>/exe` needs `PTRACE_MODE_READ` (same uid, or `CAP_SYS_PTRACE`); a daemon deployed under a third uid gets an empty chain again, by design (fails closed, not open) -- see the daemon-uid section of the design note. That is deployment work, tracked separately, not a gap in this code. |
| Nothing about the measurement is cached, on macOS or Linux | `lease.rs::{SysProcessTable,measure_running_image}` (no cache type exists on either platform) | Behavioural, by construction: the `FileStamp` cache and `measure_executable` are deleted, so every gated command re-measures every ancestor. This closes R3-F2 by removing what it exploited (every stamp field was owner-settable, so a same-length in-place rewrite plus `utimensat` was served the pre-rewrite identity for the daemon's lifetime) rather than by adding `ctime` to the stamp. Cost of having no cache, measured (release, M-series): 6.3ms for a real 6-deep chain per gated command, against 40-90ms for spawning `op --version` alone. **RAI-32, 2026-08-23, Linux cost measured (not assumed) and stated honestly because it is a real divergence from the macOS number:** a cdhash lookup is O(1) in file size; a Linux content hash is not. BLAKE2b-256 over the whole file, this box: 0.5ms for a 43KB CLI (`sleep`), 5.8ms for a 3.3MB release binary (comparable to macOS's per-ancestor cost), 182-265ms for a 100-120MB interpreter (`node`-scale). A shell-and-CLI ancestor chain costs about what macOS costs; an interpreter-scale ancestor in the chain does not, and is a real, user-visible per-call cost this fill accepts rather than caching around. A `(dev, ino)` cache keyed off the already-open fd's `fstat` would be sound here (the inode cannot recycle while the ancestor process still holds it open), and is recorded as a considered, not-built option in `docs/design/linux-code-identity.md` for a follow-up if interpreter-scale ancestors turn out to be common on real gated chains. |
...
| Argv is passed to the child as **separate argv entries, never a shell string** (no shell-injection surface); the real binary is resolved by the operator's config pin when set, else via PATH skipping the shim | `provider.rs::{OpProvider,EnvFileProvider}::run` (`Command::new(real).args(...)`), `paths::{resolve_command,find_real,find_real_op}` | `daemon.rs::env_file_command_runs_gated_and_injects_env`, `provider.rs::run_streams_op_child_output_to_the_caller_fd` |
...
| The **daemon-up** path is immune to **caller PATH poisoning**: the `Run` frame carries only `argv`/`cwd`/`proxy_depth`/fds, never the caller's `PATH`/env, so the daemon resolves the real binary from its **own** trusted inputs — the operator's explicit per-command pin (`config.json`'s `binaries`, authored CLI-side by `sigil-config binary set`) if there is one, else its own `PATH`, pinned by the supervision definition (the launchd plist on macOS, `~/.sigil/supervisor.conf` elsewhere) — never a `PATH` inherited from whichever shell ran `sigil up`; a caller cannot redirect what the daemon spawns or steer the injected SA token to an attacker binary. The daemon-**down** path uses the caller's `PATH` but injects **no** secret (transparent exec), so a poisoned `PATH` there just runs the caller's own binary with no token, identical to no-Sigil | `local.rs::Frame::Run` (no env field), `provider.rs::OpProvider::resolve` -> `paths::resolve_command` (daemon env), `shim.rs::exec_real` (no env injected) | `paths::a_pin_reaches_a_binary_the_daemon_path_cannot_see`, `provider::a_config_pin_runs_a_binary_the_path_could_never_resolve`; plus reviewed by inspection for the frame's absent env field |
| An operator's **binary pin** narrows resolution rather than widening it, and **never falls back**. `config.json`'s `binaries` names one absolute file per command; a pin that stops resolving (removed, replaced by a directory, execute bit cleared, made relative) **refuses the run at exit 127** instead of resolving something else off the daemon's `PATH`. This is the direction that matters: a pin exists because the operator does not want the daemon choosing, so a silent substitution would reintroduce exactly the attack the pinned `PATH` prevents. The alternative shape — appending the site's directory to the daemon's `PATH` — was rejected for the same reason: it makes every present and future file in that directory spawnable with an approved credential, where a pin makes one file spawnable | `paths.rs::resolve_command` / `resolve_command_in` (no fallback arm), `provider.rs::{OpProvider::run,spawn_with_env}` (127 on `Err`), `config.rs::validate_binary_pin` (authoring-time check) | `paths::a_broken_pin_refuses_and_never_falls_back_to_the_path_walk`, `paths::a_pin_at_a_non_executable_or_a_directory_refuses`, `paths::a_relative_pin_refuses_rather_than_resolving_against_the_daemons_cwd`, `provider::a_broken_pin_refuses_the_run_rather_than_spawning_something_else`, `config::a_pin_is_validated_against_the_filesystem_before_it_is_stored` |
| A **binary pin cannot be used to bypass the shim exclusions.** Both of `find_real`'s rules are re-applied to a pinned path: a pin resident in the proxy dir (rule 2, catching a hard copy canonicalisation cannot see) and a pin canonicalising to the running Sigil binary or an alias target (rule 1) are refused with `PinIsShim`, rather than spawned into the depth-fuse loop. A pin is a way to name a file the `PATH` walk cannot reach, not an opt-out of what the walk checks. Config integrity is the pin's trust boundary, which is the boundary `config.json` already is: the daemon only ever **reads** it, at arm time, and anyone who can rewrite it can already author a rule that gates or ungates anything | `paths.rs::resolve_command_in` (proxy-dir + canonical-identity arms), `config.rs::Config::binaries` (CLI-side store, daemon read-only) | `paths::a_pin_cannot_be_used_to_bypass_the_shim_exclusions`, `paths::no_pin_is_the_unchanged_path_walk_including_its_exclusions` |
