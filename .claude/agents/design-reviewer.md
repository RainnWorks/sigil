...
- **The security inversion**: deny is always one calm tap; approve is one tap plus a mandatory hardware biometric (Face ID / Secure Enclave). What a rule may do is set by its lease policy (run-once vs leasable), never a risk tier. Scaling the approve gesture by "risk" (slide or hold) is itself a blocking finding, as is anything that makes denying harder or approving more casual than a single tap.
- **Motion is rationed**: the gauge (a real clock), the approve capsule press-in with its commit haptic and symbol change, the 400ms lockdown seal. Approve is a tap, never a drag track. Everything else is platform motion. Reduced-motion alternatives are mandatory.
